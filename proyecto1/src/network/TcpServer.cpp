...
void TcpServer::listenForever(const char* port) {
  ...
  // this->acceptAllConnections(Queue<Socket>* socket_queue);
  // To do: figura out what to do here, why is this here
}
...
void TcpServer::acceptAllConnections(Queue<Socket>* socket_queue) {
  while (true) {
    this->acceptConnectionRequest(socket_queue);
  }
}

void TcpServer::acceptConnectionRequest(Queue<Socket>* socket_queue) {
  ...
  this->handleClientConnection(client, socket_queue);
}
